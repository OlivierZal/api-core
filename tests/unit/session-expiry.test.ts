...
describe(isSessionExpired, () => {
  ...
  it('interprets an offset-less timestamp in the supplied IANA zone', () => {
    // 2026-04-11T12:00:00Z is 21:00 in Tokyo and 05:00 in Honolulu: the
    // same offset-less string reads expired in one zone and live in the
    // other, which is exactly the drift the `zone` parameter closes.
    expect(isSessionExpired('2026-04-11T14:00:00', 0, 'Asia/Tokyo')).toBe(true)
    expect(isSessionExpired('2026-04-11T14:00:00', 0, 'Pacific/Honolulu')).toBe(
      false,
    )
  })

  it('lets an explicit offset override the supplied zone', () => {
    // `offset: 'use'` keeps offset-bearing inputs absolute: one hour in
    // the future stays in the future whatever zone rides along.
    expect(isSessionExpired('2026-04-11T13:00:00Z', 0, 'Asia/Tokyo')).toBe(
      false,
    )
  })

  it('parses ISO timestamps without explicit timezone offset', () => {
    // Regression for upstreams that persist expiry with no `Z` and no
    // offset. Without an explicit zone these are interpreted in the
    // runtime's system timezone; a consumer threads its configured zone
