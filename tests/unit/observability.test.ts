...
describe('sensitive data redaction', () => {
  ...
  // The vocabulary is injected, not owned: a consumer's protocol key
  // (here a made-up context header) rides the SAME engine the base
  // vocabulary uses, through the shells' redaction parameter.
  it('redacts an injected protocol key in request headers', () => {
    const redaction = createRedaction(['x-mitscontextkey'])
    const config = createConfig({
      headers: {
  ...
  it('leaves a protocol key alone when no vocabulary injects it', () => {
    const config = createConfig({ headers: { 'X-MitsContextKey': 'abc123' } })
    const call = new APICallRequestData(config)
    const headers = defined(parseLog(call.toString()).headers)

    expect(headers['X-MitsContextKey']).toBe('abc123')
  })

  it('redacts an injected protocol key in response data', () => {
    const redaction = createRedaction(['x-gizwits-user-token'])
    const response = createResponse({
      headers: { 'x-custom': 'visible', 'x-gizwits-user-token': 'tok' },
    })
    const call = new APICallResponseData(response, undefined, redaction)
    const headers = defined(parseLog(call.toString()).headers)
  ...
  it('redacts sensitive keys inside form-encoded string bodies', () => {
    ...
    // the redaction engine, the entire body — including `password=...`
    // and `username=...` — would leak verbatim into the request log
    // lines.
    const config = createConfig({
...
describe(createAPICallErrorData, () => {
  ...
  it('serializes with the injected vocabulary', () => {
    const redaction = createRedaction(['x-proto-secret'])
    const error = new HttpError('Request failed', {
      config: createConfig(),
      redaction,
      response: {
        data: {},
        headers: { 'x-proto-secret': 'tok', 'x-trace': 'keep' },
        status: 500,
      },
    })
    ...
    expect(headers['x-proto-secret']).toBe('******')
