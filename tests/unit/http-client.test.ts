...
describe('httpError', () => {
  ...
  // Security regression guard (melcloud-api user report, 2026-08-21):
  // the request snapshot rides inside the thrown error into every host
  // logger and into the diagnostic reports users paste into issues, and
  // it used to carry `Authorization: Bearer …` verbatim. The injected
  // vocabulary must ride the same seat as the base one.
  it('redacts secret-bearing request headers and leaves the others verbatim', () => {
    const error = new HttpError('boom', {
      ...
      redaction: createRedaction(['x-proto-key']),
  ...
  // An upstream can answer a refusal as JSON TEXT and the failed body
  // is kept raw — a token-bearing field inside the text must already
  // read `******` when a logger receives it.
  it('redacts token-bearing fields inside a raw JSON error text', () => {
    const error = new HttpError('boom', {
      config: { method: 'POST', url: '/connect/token' },
      response: {
        data: '{"error":"invalid_grant","token":"tok-123"}',
        headers: {},
        status: 400,
      },
    })
  ...
  // A token can ride inline in the URL (`?token=…`) rather than in the
  ...
  it('redacts the token-bearing query of the request URL', () => {
    const error = new HttpError('boom', {
      config: { method: 'GET', url: '/callback?token=auth-code&state=xyz' },
      response: { data: null, headers: {}, status: 400 },
    })

    expect(error.config?.url).toBe('/callback?token=******&state=xyz')
  })
...
describe(HttpClient, () => {
  ...
  // The end-to-end clause that would have caught the 2026-08-21 leak:
  // the error a real request throws must not carry the bearer token
  // that request just sent — default headers and per-request headers
  // alike, base vocabulary and injected vocabulary alike.
  ...
  // Without an injected vocabulary the client still redacts the base
  // carriers — the default seat this package guarantees.
  it('falls back to the base vocabulary when none is injected', async () => {
    mockFetch.mockResolvedValueOnce(
      mockFetchResponse({ err: 'denied' }, {}, 403),
    )

    const promise = createClient().request({
      headers: { Authorization: 'Bearer secret', 'X-Proto-Key': 'kept' },
      url: '/guarded',
    })

    await expect(promise).rejects.toMatchObject({
      config: { headers: { Authorization: '******', 'X-Proto-Key': 'kept' } },
    })
  })
  ...
  it('preserves the base path when joining an absolute-style url', async () => {
    const client = new HttpClient({
      baseURL: `${BASE_URL}/Some.Prefix.Client`,
      timeout: 0,
    })
    mockFetch.mockResolvedValueOnce(mockFetchResponse({}, {}, 200))
    ...
    expect(extractUrl()).toBe(`${BASE_URL}/Some.Prefix.Client/User/ListDevices`)
  })
