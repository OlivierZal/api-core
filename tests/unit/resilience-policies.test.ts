...
import { HttpStatus } from '../../src/http/index.ts'
import {
...
// The auth-failure vocabulary is injected, not owned: a wire that
// reports an expired token as HTTP 400 (heatzy-api's Gizwits upstream)
// passes both statuses; the default stays 401-only.
describe('authRetryPolicy with injected statuses', () => {
  it('replays a 400 when the consumer passes it as an auth-failure status', async () => {
    using guard = new RetryGuard(1000)
    const reauthenticate = vi
      .fn<() => Promise<boolean>>()
      .mockResolvedValue(true)
    ...
    const attempt = vi
      .fn<() => Promise<string>>()
      .mockRejectedValueOnce(createServerError(400, '/x'))
      .mockResolvedValueOnce('retried')
  ...
  it('passes a 400 through under the default 401-only vocabulary', async () => {
    using guard = new RetryGuard(1000)
    const reauthenticate = vi.fn<() => Promise<boolean>>()
    const policy = new AuthRetryPolicy(guard, reauthenticate)
