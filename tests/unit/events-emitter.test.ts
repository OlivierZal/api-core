...
// The consumer-facing sync notification: unlike the `emit*` family it
// is awaited, so a decorated mutation resolves only after the observer
// has run — with the same non-throwing contract.
describe('emitSyncComplete', () => {
  it('resolves without a callback configured', async () => {
    const logger = createLogger()
    const emitter = new LifecycleEmitter({}, logger)

    await expect(emitter.emitSyncComplete()).resolves.toBeUndefined()
    expect(logger.error).not.toHaveBeenCalled()
  })

  it('awaits the callback and forwards the sync parameters', async () => {
    const logger = createLogger()
    const onSyncComplete = vi
      .fn<(params?: { ids?: string[] }) => Promise<void>>()
      .mockResolvedValue()
    const emitter = new LifecycleEmitter({ onSyncComplete }, logger)

    await emitter.emitSyncComplete({ ids: ['a', 'b'] })

    expect(onSyncComplete).toHaveBeenCalledWith({ ids: ['a', 'b'] })
    expect(logger.error).not.toHaveBeenCalled()
  })

  it('logs and swallows a rejecting callback', async () => {
    const logger = createLogger()
    const onSyncComplete = vi
      .fn<() => Promise<void>>()
      .mockRejectedValue(new Error('observer boom'))
    const emitter = new LifecycleEmitter({ onSyncComplete }, logger)
