...
/**
 * Structural host contract of the {@link setting} decorator: an
 * optional external store the accessor delegates to.
 * @internal
 */
  ...
  return {
    ...
    set(this: HasSettingManager, value: string): void {
      ...
      if (settingManager !== undefined) {
        ...
        // field (credentials, tokens, context key, expiry, login
        // backoff): delete the key outright when the host delegates
        // `unset`, so a logout leaves no empty leftovers. `get` falls
        // back to the accessor's own `''` default when the key is
        // absent, so this reads back identically.
