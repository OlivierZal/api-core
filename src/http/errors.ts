...
// header: a sign-in puts the account's password in the request BODY,
// and an upstream echoes the credential it just rejected.
const redactHeaders = <T>(
  ...
  Object.fromEntries(
    Object.entries(headers).map(([key, value]) => [
      key,
      redaction.isSensitive(key) ? REDACTED : value,
    ]),
  )
...
const redactParams = (
  ...
  Object.fromEntries(
    Object.entries(params).map(([key, value]) => [
      key,
      redaction.isSensitive(key) ? REDACTED : redaction.redactValue(value),
    ]),
  )
 ...
 * rejected, and the SDKs' own loggers have always blanked it. It is
 * therefore typed `unknown`, since nothing may rely on the shape of a
 * value whose secrets have been replaced.
 *
 * The vocabulary the redaction applies is the injected
 * {@link Redaction} — the consuming SDK passes the engine bound to its
 * wire's credential keys; without one, the base vocabulary applies.
 * @category HTTP
 */
export class HttpError extends Error {
   ...
   * @param options.redaction - Redaction engine carrying the consumer's sensitive-key vocabulary.
   */
  public constructor(
    message: string,
    options: {
    ...
    const { config, redaction = baseRedaction, response } = options
    this.name = 'HttpError'
    this.response = {
      ...response,
      data: redaction.redactValue(response.data),
      headers: redactHeaders(redaction, response.headers),
    }
    this.config =
      config === undefined
        ? config
        : {
            ...
            ...(config.data !== undefined && {
              data: redaction.redactValue(config.data),
            }),
            ...(config.headers !== undefined && {
              headers: redactHeaders(redaction, config.headers),
            }),
            ...(config.params !== undefined && {
              params: redactParams(redaction, config.params),
            }),
            ...
            ...(config.url !== undefined && {
              url: redaction.redactUrl(config.url),
            }),
          }
  }
}
