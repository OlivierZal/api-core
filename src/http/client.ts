import { type Redaction, baseRedaction } from '../observability/redaction.ts'
...
// RFC 3986 and replaces base's path entirely — which drops any prefix
// (e.g. `/app`) from `baseURL`. Strip the leading slash so the
 ...
 * `getSetCookie()`, so it is merged explicitly.
 * @param headers - Headers of a fetch `Response`.
 * @returns Plain header record, `set-cookie` kept as an array.
 * @category HTTP
 */
 ...
 * layers stay unchanged when the transport is swapped. Every thrown
 * {@link HttpError} snapshot is redacted through the client's injected
 * {@link Redaction} vocabulary.
 * @category HTTP
 */
export class HttpClient {
   ...
   * and optional default headers / undici dispatcher / redaction
   * vocabulary.
