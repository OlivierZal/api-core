 ...
 *
 * Used by the consuming API clients to honor HTTP 429 responses:
 * record the rate-limit on the error path (respecting `Retry-After`
 * when present) and consult `isPaused` / `remaining` on the request
 * path to fail fast without hammering the upstream server.
 */
