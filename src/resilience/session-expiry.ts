 ...
 *
 * Offset-less ISO strings — the format some upstreams persist their
 * expiry in — are interpreted in the supplied `zone`. Without a zone,
 * the runtime's system timezone is used. Strings with an explicit `Z`
 * or `±HH:MM` offset are parsed as absolute `Temporal.Instant`s,
 * ignoring `zone`.
